...
const OptionDropdown = ({ label, options, value, onChange }) => {
  ...
  const handleSelect = (option) => {
    onChange(option.value); // Mengirim nilai option.value ke onChange
    setIsOpen(false);
  };

  return (
    <div className="flex flex-col relative">
      <label className="block text-sm font-semibold text-[#1B0947]">
        {label}
      </label>
      <div className="relative">
        <div
          onClick={toggleDropdown}
          className="rounded-md bg-white flex justify-between items-center cursor-pointer placeholder-[#5B25D9] placeholder:text-sm text-first placeholder-opacity-50 mt-1 p-2 w-full border:opacity-50 border border-[#5B25D9] border-opacity-50 focus:border-[#5B25D9] focus:outline-none focus:shadow-lg transition-colors duration-300"
        >
          <div className="flex gap-2">
            ...
            <span className="w-28">
              {/* Menampilkan label dari opsi yang sesuai dengan nilai value */}
        ...
        <div
          className={`absolute top-full left-0 bg-white border border-gray-300 rounded-md mt-1 w-full z-10 ${
            isOpen ? "" : "hidden"
          }`}
        >
          {options.map((option) => (

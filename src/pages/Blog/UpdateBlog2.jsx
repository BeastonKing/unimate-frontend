...
const UpdateBlog = () => {
  ...
  const [content, setEditorHtml] = useState(null);
  ...
  const handleImageUpload = (e) => {
    ...
    if (file) {
      ...
      reader.onload = () => {
        setImageUrl(reader.result); // Update imageUrl state
    ...
    if (imageUrl) {
      fetch(imageUrl)
        ...
        .then((blob) => {
          ...
          axios
            .put(
              `${import.meta.env.VITE_UNIMATE_BE_SERVICES}/api/blog/update`,
              formData,
              {
                headers: {
            ...
            .then((response) => {
              ...
              window.alert(
                'Article has been Updated. Redirecting to Blog Management Page.'
              );
              setTimeout(() => {
  ...
  return (
    <div className='flex justify-center items-center h-full'>
      <div className='container px-4 max-w-screen-lg'>
        <div
          className={`bg-${backgroundColor} h-[300px] mt-12 sm:mt-0 sm:h-[469px] w-full relative`}
          style={{ backgroundImage: imageUrl ? `url(${imageUrl})` : 'none' }}
        >
          {/* Overlay for file input */}
          <div className='m-10 absolute inset-x-0 bottom-0 flex justify-start'>
            <label
              htmlFor='file_input'
        ...
        <div className='mt-10'>
          <input
            ...
            value={title}
          />
          {/* TODO, Text editor cannot be editted */}
              ...
              >
                {[...Array(15)].map((_, i) => (
                  <option
                    key={i}
                    value={i + 1}
                    selected={article && article.readingTime == i + 1}
                  >
                    {`${i + 1} Min`}
                  </option>
                ))}
          ...
          <div className='mt-6 bg-gray-200 flex items-center justify-between p-4'>
            ...
            <div className='relative'>
              ...
              >
                <option
                  value='INFORMASI_BEASISWA'
                  selected={
                    article && article.blogType === 'INFORMASI_BEASISWA'
                  }
                >
                  Informasi Beasiswa
                </option>
                <option
                  value='PEKERJAAN_LUAR_NEGERI'
                  selected={
                    article && article.blogType === 'PEKERJAAN_LUAR_NEGERI'
                  }
                >
                ...
                <option
                  value='TIPS_TRIK'
                  selected={article && article.blogType === 'TIPS_TRIK'}
                >
                  Tips & Trick
                </option>
              </select>
